...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.visited = ()
        self.start_state = (self.startingPosition, self.visited)
...
def cornersHeuristic(state, problem):
    ...
    while True:
        if len(unvisited) > 0:
            minimumDist = util.manhattanDistance(current_pos, unvisited[0])
            temp = unvisited[0]
            for i in unvisited[1:]:
                if util.manhattanDistance(current_pos, i) < minimumDist:
                    minimumDist = util.manhattanDistance(current_pos, i)
                    temp = i 
            current_pos = temp
            unvisited.remove(temp)
            sum_path = sum_path + minimumDist
            minimumDist = 0
        else:
            break
...
def foodHeuristic(state, problem):
    ...
    for i in l:
        if (position, i) in problem.heuristicInfo:
            maxdist = max(maxdist, problem.heuristicInfo[(position, i)])
        else:
            d = mazeDistance(position, i, problem.startingGameState)
            maxdist = max(maxdist, d)
            problem.heuristicInfo[(position, i)] = d
    return maxdist
